...
- **规划(停滞复盘与方向拍板)**：
  - 对停滞期（08-28 IP 圣经提交后至今）做全面摸底：git 提交链、任务单、阶段 0 差距
    评估、IP 圣经 v0.1 与全库模块/测试数据，确认停滞根因为"方向断层（愿景与任务
    单断链）+ 结构卡死（商业验证依赖外部资源）+ 工程能力 > 产品验证"。
  - **方向拍板**：确认走 XIAOYO 陪伴路线（圣经 v0.1 有效）；`server/` Go 后端保留
    现状（西游记分发 + 云端 TTS 继续服务）；第一切入点为代码资产盘点。
- **盘点(代码资产盘点，圣经行动项一落地)**：
  - 新增 `DevelopmentPlan/20260910_代码资产盘点.md`：全库 12 个模块逐一登记行数、
    测试映射与处置分级（保留 8 / 改造 5 / 冻结 1）。
  - **核心结论**：业务代码约 1.6 万行 + 测试 1.26 万行（755 用例，测试/代码比
    约 0.76），资产与新方向高度兼容——圣经 4.3 唯一"砍"建议（author）降级为
    冻结，`server/` 保留后，XIAOYO 世界的搭建是增量改造而非推倒重来。
  - **就绪度确认**：首次启动 + 深夜读书两场景所需资产（companion 语义/触发器、
    dashboard 首屏、library 空书架、reader 正文流、ambient 氛围音、CyberToast
    拟人气泡）全部就绪，零新增依赖。
  - **硬缺口登记**：原创 Rive 母版缺位（社区 `.riv` 禁商用），短期吉祥物走
    Canvas 自绘路线（`board_mascot` / `xiaoyo_mascot`）；台词库 0 句为产品灵魂
    空转点，列为下一步第一优先。
  - **验证**：纯文档产出，未触碰业务代码，工程门禁不受影响。README 描述的是
    既有功能与架构，本次无功能/架构变更，无需更新；待"台词 + 两场景"落地后
    再按新定位改写 README 产品叙事。

## **2026-08-13**

- **评估(阶段 0 落地差距核查)**：
  - 对「阶段 0 审批方案」声称的工程前置产物逐项核对仓库真实文件：PROD-00 功能
    开关、PERF-0-A 帧统计框架、三根导航壳、七态听读首页、Xiaoyo 领域/Provider/桥、
    作者听校本地领域、手机 HTML 原型、原型合同检查脚本、真机采集与基线比较工具、
    IP-0 清单、商业证据计算器、Xiaoyo 候选概念图均真实存在，文档未虚报。
  - 同时确认未完成项也属实：`lib/features/commerce/` 不存在、`xiaoyo_v2_base.riv`
    不存在、IP-0 六类证据均为 `complete: false`、本机 `adb devices` 无设备、
    真实支付/退款/台账/访谈/作者样本均未提供，详设与收口文档对此诚实标注。
  - **结论**：工程侧确实落地；但阶段 0 的核心是商业验证，真实用户、真实支付、
    真机性能、原创 IP 资产均依赖仓库之外的外部资源，当前工作区无法独立闭环，
    用户"落地不了"的判断成立。
...
- **维护（部署工作流同步）**：按服务器新目录结构统一阅游生产运行目录为 `/www/wwwroot/yueyou/`，补充 `server/` 交叉编译、上传、重启 `yueyou.service`、状态检查和公网接口验证步骤；同步更新 `AGENTS.md`、`.windsurf/rules/AGENT.md`、发布守卫及相关工作流。
- **验证**：仅完成文档路径检索与 Markdown 变更检查；未执行远程上传、systemd 重启或公网探活。
